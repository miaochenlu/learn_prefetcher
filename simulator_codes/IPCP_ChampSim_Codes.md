# IPCP ChampSim Code Reading

# 1. 成员变量

## 1. IP table

<div align="center"><img src="attachments/Pasted%20image%2020230411133525.png" width="500"></div>
...
table里面的每个entry存一个delta和一个confidence
...
<div align="center"><img src="attachments/Pasted%20image%2020230411141820.png" width="500"></div>
	...
	if(((curr_page << 1) | (line_offset >> 5)) == rstable[cpu][c].region_id) {
		...
		if(rstable[cpu][c].trained_dense == 0) {
			...
			for(int i = 0; i < NUM_OF_LINES_IN_REGION; i++)
				if(rstable[cpu][c].line_access[line_offset 
					& REGION_OFFSET_MASK] == 1)
					count++;
			// 75% of the cache lines in the region are accessed.
			if(count > 24)     
		...
		if(rstable[cpu][c].tentative_dense == 1 
			|| rstable[cpu][c].trained_dense == 1) {
...
if (trackers_l1[cpu][index].str_valid == 1) { // stream IP
 // for stream, prefetch with twice the usual degree
	...
	for (int i = 0; i < prefetch_degree[cpu][1]; i++) {
		...
		// Issue prefetch request only if prefetch address 
		// is not present in RR filter
...
trackers_l1[cpu][index].conf = update_conf(stride, 
...
```cpp
/*
If the actual stride and predicted stride are equal, 
then the confidence counter is incremented. 
*/
...
```cpp
if (trackers_l1[cpu][index].conf > 1 
	&& trackers_l1[cpu][index].last_stride != 0 && flag == 1) {
        meta_counter[cpu][1]++;
        total_count[cpu]++;
        ...
        for (int i = 0; i < prefetch_degree[cpu][2]; i++) {
            uint64_t pf_address = (line_addr + 
				(trackers_l1[cpu][index].last_stride * (i + 1))) << LOG2_BLOCK_SIZE;
...
CSPT_l1[cpu][last_signature].conf = update_conf(stride, 
 CSPT_l1[cpu][last_signature].stride, CSPT_l1[cpu][last_signature].conf);
...
if (CSPT_l1[cpu][signature].conf >= 0 
	&& CSPT_l1[cpu][signature].stride != 0 && flag == 1) {
...
if (num_prefs == 0 && spec_nl[cpu] == 1) {
	if (flag_nl[cpu] == 0) flag_nl[cpu] = 1;
	else {
...
if (num_misses[cpu] % 256 == 0 && cache_hit == 0) {
	mpki[cpu] = ((num_misses[cpu] * 1000.0) 
...
for (int i = 0; i < 5; i++) {
	if (pref_filled[cpu][i] % 256 == 0) {
		acc_useful[cpu][i] = acc_useful[cpu][i] / 2.0 
			+ (pref_useful[cpu][i] - acc_useful[cpu][i]) / 2.0;
		acc_filled[cpu][i] = acc_filled[cpu][i] / 2.0 
			+ (pref_filled[cpu][i] - acc_filled[cpu][i]) / 2.0;
